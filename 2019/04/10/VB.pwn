...
public OnPlayerText(playerid, text[])
{
    ...
    if(IsFlooding[playerid] == 3 && !IsPlayerAjudante(playerid)){
        SendClientMessage(playerid, Verde, "| ANTI-FLOOD | Você foi calado(a) por 30 segundos!");
        ...
        SendClientMessage(playerid, Verde_Escuro, "| ANTI-FLOOD | Pare com flood, na próxima vocé será calado(a)!");
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    if(IsFloodingCommand[playerid] == 3 && !IsPlayerAjudante(playerid)){
        ...
        SendClientMessage(playerid, Verde_Escuro, "| ANTI-FLOOD | Pare com flood, na próxima vocé será bloqueado(a) de usar comandos!");
    ...
    if(strcmp(cmd, "/aviso", true)==0)
	{
        if(IsPlayerAjudante(playerid))
        {
            ...
            SendClientMessage(playerid, Erro, "| ERRO | O(A) Jogador(a) não conectado(a)!");
            return 1;
            }
    ...
    if(strcmp(cmd,"/cadeia", true)==0)
    {
        if(IsPlayerAjudante(playerid))
        {
            ...
            SendClientMessage(playerid, Erro, "| ERRO | O(A) jogador(a) está preso na municipal!");
            return 1;
            }
	...
	if(strcmp(cmd, "/soltar", true)==0)
	{
        if(IsPlayerAjudante(playerid))
        {
            ...
            MessageFormat(playerid, Erro, "| ERRO | O(A) Jogador(a) %s[%d] não está preso(a)", PlayerName(id), id);
            return 1;
            }
    ...
    if(strcmp(cmd, "/cv", true)==0)
    {
        if(IsPlayerAjudante(playerid))
        {
            ...
            SendClientMessage(playerid, Erro,"| ERRO | Você está em um veiculo!");
            return 1;
            }
	...
	if(strcmp(cmd, "/reparar", true) == 0)
    {
        if(IsPlayerAjudante(playerid) || IsPlayerVip(playerid))
        {
            ...
            SendClientMessage(playerid, Erro, "| ERRO | Você está em perseguição e não poderar reparar o veiculo!");
            return 1;
            }
            ...
            if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 510)
            {
                if(IsPlayerInRangeOfPoint(playerid, 20.0, 784.9813, -1345.0000, 13.1522))
                {
                    if(Jornal[playerid]==0)
                    {
                        ...
                        SendClientMessage(playerid, Branco, "| JORNAL | Você pegou 20 jornais, vá até o checkpoint {FF0000}vermelho {FFFFFF}marcado mo mini-mapa");
                    } else {
                        SendClientMessage(playerid, Erro, "| ERRO | Você já pegou jornal!");
                    }
                } else {
    ...
    if(strcmp(cmd, "/perseguicao", true)==0)
    {
        if(PlayerInfo[playerid][Profissao] == PoliciaMunicipal)
        {
            ...
            if(playerid == id){
            SendClientMessage(playerid, Erro, "| ERRO | Você não pode da perseguição em você mesmo!");
            return 1;
            }
            ...
            SendClientMessage(playerid, Erro, "| ERRO | O(A) Jogador(a) já está em perseguição!");
            return 1;
            }

            MessageFormatAll(0x2894FFAA, "| PERSEGUIÇÃO | O(A) Policial %s[%d] iniciou uma perseguição no(a) jogador(a) %s[%d] por não se render!", PlayerName(playerid), playerid, PlayerName(id), id);
            Perseguicao[id]=true;
    ...
    if(strcmp(cmd, "/localizar", true)==0)
    {
        if(PlayerInfo[playerid][Profissao] == PoliciaMunicipal)
        {
            ...
            if(id==playerid){
            SendClientMessage(playerid, Erro, "| ERRO | Você não pode executar comando em você mesmo");
            return 1;
            }
    ...
    if(strcmp(cmd, "/abordar", true) == 0)
	{
        if(PlayerInfo[playerid][Profissao] == PoliciaMunicipal)
        {
            if(IsPlayerUniformePM(playerid))
            {
                ...
                SendClientMessage(playerid, Erro, "| ERRO | O(A) Jogador(a) mão está procurado(a)!");
                return 1;
                }
    ...
    if(strcmp(cmd, "/prender", true) == 0)
    {
        if(PlayerInfo[playerid][Profissao] == PoliciaMunicipal)
        {
            if(IsPlayerUniformePM(playerid))
            {
                ...
                MessageFormat(id, 0x2894FFAA, "| PRISÃO | Você foi preso pelo Policial %s ( Motivo: Procurado(a) )", PlayerName(playerid));
                MessageFormat(playerid, 0x2894FFAA, "| POLICIA | Você prendeu o(a) Jogador(a) %s ( Procurado(a) )", PlayerName(id));
                MessageFormat(playerid, 0x2894FFAA, "| POLICIA | Você recebeu a recompensa de $%d por prender um jogador(a) que estava sendo procurado(a) pela policia", PlayerInfo[id][Estrelas]*100);
    ...
    if(strcmp(cmd, "/planejarroubo", true)==0)
    {
        if(PlayerInfo[playerid][Profissao] == LadraoGas)
        {
            ...
            SendClientMessage(playerid, Branco, "| INFO | Você Planejou 10 roubo de gás, vá até o checkpoint {FF0000}vermelho {FFFFFF}marcado mo mini-mapa");
    ...
    if(strcmp(cmd, "/radio", true)==0)
    {
        ...
        SendClientMessage(playerid, Branco, "| RÁDIO | Para pausar digite: /radio stop ou espaço apenas!");
        return 1;
    }
    ...
    if(strcmp(cmd, "/comandos", true)==0)
    {
        ...
        strcat(string, "{FFFFFF}Aperte '{4AA5FF}F{FFFFFF}' ou '{4AA5FF}Enter{FFFFFF}' para abrir o portão do derpartamento\n\n");
    	...
    	new string[1000];
        strcat(string, "{FF8080}/PlanejarRoubo {FFFFFF}- Para começar a roubar os gás \n");
        strcat(string, "{FF8080}/PegarGas {FFFFFF}- Pegue o gás \n");
        strcat(string, "{FF8080}/CancelarRoubo {FFFFFF}- Cancele o roubo \n");
        strcat(string, "{FF8080}/ColocarGas {FFFFFF}- Coloque o gás no caminhão \n");
        strcat(string, "{FF8080}/Hq {FFFFFF}- Localize seu trabalho \n");
        strcat(string, "{FF8080}/Cp [texto] {FFFFFF}- Chat profissão \n");
        ShowPlayerDialog(playerid, LadraoGas, DIALOG_STYLE_MSGBOX, "{FF0000}Ladrão de Gás", string, "Fechar", "");
        }
        return 1;
	}
    if(strcmp(cmd, "/trabalhar", true)==0)
    {
        ...
        SendClientMessage(playerid,C_EntregadorJornal,"| PROFISSÃO | Para saber os comandos da profissão, digite: /Comandos");
        ...
        SendClientMessage(playerid,C_Taxi,"| PROFISSÃO | Para saber os comandos da profissão, digite: /Comandos");
        ...
        SendClientMessage(playerid,C_PoliciaMunicipal,"| PROFISSÃO | Apartir de agora você ira exerce a profissão Policia Municipal");
        SendClientMessage(playerid,C_PoliciaMunicipal,"| PROFISSÃO | Para saber os comandos da profissão, digite: /Comandos");
        ...
        SendClientMessage(playerid,C_Corregedoria,"| PROFISSÃO | Para saber os comandos da profissão, digite: /Comandos");
        ...
        SendClientMessage(playerid,C_LadraoGas,"| CARGO | Apartir de agora você ira exerce o cargo Ladrão de Gás");
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    for(new v; v < sizeof(BKJORNAL); v ++) if(vehicleid == BKJORNAL[v])
    {
        ...
        {
            ClearAnimations(playerid);
            return SendClientMessage(playerid, Erro,"| ERRO | Veiculo exclusivo da profissão Entregador(a) de Jornal!");
        }
    }
    for(new v; v < sizeof(VCPOLICIA); v++) if(vehicleid == VCPOLICIA[v])
    {
        ...
        {
            ClearAnimations(playerid);
            return SendClientMessage(playerid, Erro,"| ERRO | Veiculo exclusivo da profissão Policia!");
        }
        if(!ispassenger && vehicleid != INVALID_VEHICLE_ID && !IsPlayerUniformePM(playerid) && PlayerInfo[playerid][Profissao] == PoliciaMunicipal)
        {
            ClearAnimations(playerid);
            return SendClientMessage(playerid, Erro,"| ERRO | Você não está com o uniforme!");
        }
    }
    for(new v; v < sizeof(CRRTAXI); v++) if(vehicleid == CRRTAXI[v] || vehicleid == CRRTAXI[v])
    {
        ...
        {
            ClearAnimations(playerid);
            return SendClientMessage(playerid, Erro,"| ERRO | Veiculo exclusivo da profissão Taxista!");
        }
    }
    ...
    for(new v; v < sizeof(CARROLG); v++) if(vehicleid == CARROLG[v] || vehicleid == CARROLG[v])
    {
        ...
        {
            ClearAnimations(playerid);
            return SendClientMessage(playerid, Erro,"| ERRO | Veiculo exclusivo do cargo Ladrão de Gás!");
        }
    }
    return 1;
}
    ...
    if(PlayerInfo[playerid][Profissao]==EntregadorJornal && IsPlayerInRangeOfPoint(playerid, 1.5, Armazenar[playerid][RandomX], Armazenar[playerid][RandomY], Armazenar[playerid][RandomZ]))
    {
        if(JornalBlock[playerid] == false)
        {
            ...
            if(Jornal[playerid] > 0)
            {
                ...
                MessageFormat(playerid, Branco, "| JORNAL | Você entregou um jornal, e agora você tem {FF0000}%d {FFFFFF}jornais!", Jornal[playerid]);
                MessageFormat(playerid, Branco, "| JORNAL | Você ganho {00C40A}$%d {FFFFFF}pela entrega localizada em {FF8000}%s", Armazenar[playerid][VaiReceber], Armazenar[playerid][RandomLocal]);
                new rand = random(sizeof(LocaisJornal));
                format(Armazenar[playerid][RandomLocal],40,LocaisJornal[rand][Local]);
                Armazenar[playerid][RandomX] = LocaisJornal[rand][JornalX];
                Armazenar[playerid][RandomY] = LocaisJornal[rand][JornalY];
                ...
                PlayerTextDrawShow(playerid, CarregandoInterior[playerid]);
                SetTimerEx("LoadInterior", segundos(2), false, "d", playerid);
                SendClientMessage(playerid, Branco, "| JORNAL | Você entregou o último jornal!");
                SendClientMessage(playerid, Branco, "| JORNAL | Para pegar mas jornais, volte para HQ e pegue mais");
            }
        } else {
            SendClientMessage(playerid, Erro, "| ERRO | Você acabou de entregar, aguarde alguns segundos...!");
        }
    }
	return 1;
}
            ...
            switch(listitem)
            {
                case 0:
                {
                    if(PlayerInfo[playerid][Level] >= 100)
                    {
                        ...
                        Profissaoon[playerid]=1;
                        Logarprof(playerid);
                        ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FF0000}Encaminhamento - {FFFFFF}Agencia de Empregos", "{FFFFFF}Você foi aceito no emprego {00FF00}Policia Municipal \n\n{FFFFFF}Para iniciar os serviços siga o checkpoint {FF0000}Vermelho {FFFFFF}em seu radar ", "Fechar", "");
                    } else {
                        SendClientMessage(playerid, Erro, "| ERRO | Você não tem level suficiente para pegar essa profissão.");
                    }
                }
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    if(playertextid == PlayerLogin[playerid][11])
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid,0,DIALOG_STYLE_MSGBOX,"{FF0000}Erro","{FFFFFF}Conta não registrada!","Fechar","");
        }
        return 1;
    }
    ...
    if(playertextid == PlayerRegistro[playerid][15]) // <<
    {
        ...
        ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FF0000}ERRO", "{FFFFFF}Não tem mais skin!", "Fechar", "");
        SkinPlayer[playerid]=0;
        }
    ...
    if(playertextid == PlayerRegistro[playerid][20])
    {
        ...
        if(!(cache_num_rows() > 0))
        {
            ...
            if(!(cache_num_rows() > 0))
            {
                ...
                ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FF0000}Erro", "{FFFFFF}Email já em uso!", "Fechar", "");
            }
        } else {
            ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FF0000}Erro", "{FFFFFF}Nome já em uso!", "Fechar", "");
        }
        return 1;
    }
    return 1;
...
{
    MessageFormatAll(0x2894FFAA, "| PERSEGUIÇÃO | O(A) jogador(a) %s[%d] não está sendo mais perseguido pela pocilia por passar 5 minutos!", PlayerName(playerid), playerid);
